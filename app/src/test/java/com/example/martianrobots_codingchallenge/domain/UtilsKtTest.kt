...
class RobotFunctionsTest {
    ...
    fun `parseGridSize should return null for invalid input`() {
        assertThat(parseGridSize("10","s10")).isNull()
        assertThat(parseGridSize("5","-41")).isNull()
        assertThat(parseGridSize("4","c")).isNull()
    }
    ...
    fun `getRobotColor should return the correct color based on orientation`() {
        assertThat(getRobotColor(Orientation.N)).isEqualTo(Color.Cyan)
        assertThat(getRobotColor(Orientation.S)).isEqualTo(Color.Yellow)
        assertThat(getRobotColor(Orientation.E)).isEqualTo(Color.White)
        assertThat(getRobotColor(Orientation.W)).isEqualTo(Color.Blue)
    }
}
